...
@router.get("/apis/{api_name}", response_model=APIInDB)
def read_api_endpoint(api_name: str, db: Session = Depends(get_db)):
    db_api = get_api(db, api_name)
    if db_api is None:
        raise HTTPException(status_code=404, detail="API not found")
    return db_api

@router.put("/apis/{api_name}", response_model=APIInDB)
def update_api_endpoint(
    api_name: str, api: APIUpdate, db: Session = Depends(get_db)
):
    db_api = get_api(db, api_name)
    if db_api is None:
        raise HTTPException(status_code=404, detail="API not found")
...
@router.delete("/apis/{api_name}", response_model=APIInDB)
def delete_api_endpoint(api_name: str, db: Session = Depends(get_db)):
    db_api = get_api(db, api_name)
    if db_api is None:
        raise HTTPException(status_code=404, detail="API not found")
